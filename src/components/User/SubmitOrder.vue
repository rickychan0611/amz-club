      ...
      >
        ...
        >
          ...
          <div class="headline mt-1 ">
            Step 1:
          </div><div class="title my-1 ">
            Place your order on Amazon
          </div>
          <div class="body-1 red--text">
            Make sure the item is sold by: {{ product.soldBy }}
          </div>
          <v-btn
            class="orange mb-4"
            rounded
            ...
            target="_blank"
          >
            Go to Amazon now &nbsp; <v-icon>mdi-forward</v-icon>
          </v-btn>
          ...
          <div class="title my-1 ">
            After purchasing, enter the order# here:
          </div>
          <v-container
            text-xs-center
            class=""
          ...
          <div class="title my-1 ">
            Upload a screenshot of your order details
          </div>
          <v-container
            text-xs-center
            class=""
          >
            ...
            />
            <v-img
              v-if="this.orderImgUrl"
              :src="this.orderImgUrl"
              height="200px"
              contain
            />
          </v-container>
          ...
          <div v-if="orderNumber.length > 0 && orderNumber.length < 19" class="caption red--text">Please check your order number</div>
          <hr>
          <v-btn
            class="blue my-4"
            dark
...
export default {
  ...
  computed: {
    ...
    formIsValid () {
      ...
      if (this.orderNumber.length == 19 && this.filePicked == true) {
        //
        return true
      } else {
        return false
  ...
  methods: {
    ...
    onPlaceOrder () {
      //console.log('xxxxx'+ JSON.stringify(this.$store.getters.user))
      let addReferral = ''
      if (this.$store.getters.user.referral == undefined || this.$store.getters.user.referral == null){
      addReferral = "none"
      }
      else{
      addReferral = this.$store.getters.user.referral
      }
      let newnewOrder = {
